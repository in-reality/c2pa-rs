...
impl Ingredient {
    ...
    /// Creates an `Ingredient` from an in-memory buffer using thread-local settings.
    ///
    /// This does not set title or hash.
    /// Thumbnail will be set only if one can be retrieved from a previous valid manifest.
    ///
    /// Retained in the in-reality fork for synchronous backend callers that still
    /// build ingredients from manifest store bytes.
    #[deprecated(
