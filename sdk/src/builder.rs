...
impl Builder {
    ...
    /// Sets pre-computed Merkle leaf hashes on the BMFF hash assertion in the Builder.
    ///
    /// Used by capture single-read signing: leaves are hashed from the input stream
    /// before `sign_file`, then injected here so the signer embeds the same Merkle
    /// roots without re-reading the asset.
